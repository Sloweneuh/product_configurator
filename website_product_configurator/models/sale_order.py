...
class SaleOrder(models.Model):
    ...
    def _website_product_id_change(self, product_id, qty=1):
 
        product = self.env['product.product'].sudo().browse(product_id)

        # Return the updated values to the website
        return {
    ...
    def _cart_update(
        ...
        if not order_line:
            ...
            values = self._website_product_id_change(product_id)
        ...
        if quantity <= 0:
            ...
            )._website_product_id_change(product_id, quantity)
            if (
...
class SaleOrderLine(models.Model):
    ...
    def _get_real_price_currency(self, product, rule_id, qty, uom, pricelist_id):
        ...
        return product.list_price, currency_id
